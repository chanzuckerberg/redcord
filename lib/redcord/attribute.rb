...
module Redcord::Attribute
  ...
  def self.included(klass)
    ...
    klass.class_variable_set(:@@custom_indexes, Set.new)
  ...
  module ClassMethods
    ...
    def custom_index(index_name, attrs)
      ...
      zadd_proc_on_redis_connection("custom_indexes", index_name, attrs.map(&:to_s))
    end
    ...
    sig { params(redis_key: String, index_name: Symbol, items_to_add: T::Array[String]).void }
    def zadd_proc_on_redis_connection(redis_key, index_name, items_to_add)
      order = 1
      Redcord::RedisConnection.procs_to_prepare << proc do |redis|
        redis.sadd("#{model_key}:#{redis_key}", index_name)
        items_to_add.each do |item_to_add|
          redis.zadd("#{model_key}:#{redis_key}_#{index_name}_attrs", order, item_to_add)
          order += 1
        end
      end
    end
